...
var calendar = function(){
    ...
    var current_resource = function(){
    	return window.location.href.match(/classrooms\/(\d+)\/bookings/)[1];
    };
    ...
    $('#calendar').fullCalendar({
        header: {
			...
			eventSources: [{  
    		url: '/classrooms/'+current_resource()+'/bookings/',  
   		}],
    ...
    eventDrop: function(booking) {
      var length = (booking.end-booking.start)/(3600000);
        ...
        function updateEvent(booking) {
              $.ajax(
                '/classrooms/'+current_resource()+'/bookings/'+booking.id,
                { 'type': 'PATCH',
    ...
    ,
      ...
      if (window.location.href.match(/new/)) {
        if(today_or_later()) {
          ...
          jQuery.post(
            '/classrooms/'+current_resource()+'/bookings',
